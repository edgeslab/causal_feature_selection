...
class CounterfactualRegressionMMD(nn.Module):
    def __init__(self,
                 n_features=None,
                 n_outputs=1,
                 pre_treatment=None,
        ...
        if sim_loss_params is not None:
            ...
            self.sim_loss_params = {"alphas": [0.1]}
    ...
    def backward(self, output, y, t):
        ...
        loss_fn = self.sim_loss(treat_rep_concat[treated].shape[0], treat_rep_concat[control].shape[0])
        sim_loss = loss_fn(treat_rep_concat[treated], treat_rep_concat[control], **self.sim_loss_params)
