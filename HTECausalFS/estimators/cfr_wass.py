...
import numpy
from pytorch.util.wasserstein import SinkhornDistance
...
class CounterfactualRegressionWass(nn.Module):
    def __init__(self,
                 n_features=None,
                 n_outputs=1,
                 pre_treatment=None,
                 ...
                 sim_loss=SinkhornDistance,  # sinkhorn distance is used for the Wasserstein error
                 sim_loss_params=None,
                 verbose=False):
        # super(CounterfactualRegression, self).__init__()
        super().__init__()
        ...
        if sim_loss_params is not None:
            self.sim_loss_params = sim_loss_params
        else:
            self.sim_loss_params = {"eps": 0.1, "max_iter": 1000, "reduction": "none"}
    ...
    def backward(self, output, y, t):
        ...
        # using maximum mean discripancy (MMD)
        loss_fn = self.sim_loss(**self.sim_loss_params)
        sim_loss, pi, C = loss_fn(treat_rep_concat[treated], treat_rep_concat[control])
        ...
        loss_pred = pred_loss_t + pred_loss_c
        loss = loss_pred + sim_loss

        if self.verbose:
            print(self.epoch, loss.item(), loss_pred.item(), sim_loss.item())
    ...
    def forward_numpy(self, x, t):
        ...
        y1 = output["yt"].detach().numpy().reshape(-1)
        y0 = output["yc"].detach().numpy().reshape(-1)
